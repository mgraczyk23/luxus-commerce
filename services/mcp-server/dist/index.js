...
// ── Backroom filter ──────────────────────────────────────────────────────────
function isBackroom(product) {
    const meta = (product.metadata ?? {});
    return (meta.is_backroom_hidden === 'true' ||
        meta.backroom === 'true' ||
        meta.is_private_room === 'true');
}
function getAttribute(product, slug) {
...
function buildMcpServer() {
    ...
    server.registerTool('search_products', {
        ...
        inputSchema: {
            ...
            limit: zod_1.z.number().int().min(1).max(50).default(20).describe('Number of results (max 50)'),
    ...
    server.registerTool('get_product', {
        ...
        inputSchema: {
            handle: zod_1.z.string().describe('Product handle/slug from the URL, e.g. "colt-python-357"'),
        },
    }, async ({ handle }) => {
        const params = new URLSearchParams({
            handle,
    ...
    server.registerTool('check_availability', {
        ...
        inputSchema: {
            handle: zod_1.z.string().describe('Product handle/slug'),
        },
    }, async ({ handle }) => {
    ...
    server.registerTool('list_brands', {
        ...
        inputSchema: {
            limit: zod_1.z.number().int().min(1).max(200).default(100).describe('Max brands to return'),
        },
    }, async ({ limit = 100 }) => {
        ...
        const brands = docs.map(b => ({
            ...
            slug: b.slug,
            country: b.country,
            description: b.description?.[0]?.children
                ? undefined
                : b.description,
    ...
    server.registerTool('get_brand_products', {
        description: 'List public products for a specific brand. Excludes backroom items.',
        inputSchema: {
            brand: zod_1.z.string().describe('Brand name, e.g. "Colt", "Smith & Wesson"'),
            limit: zod_1.z.number().int().min(1).max(50).default(20).describe('Max products to return'),
        },
    }, async ({ brand, limit = 20 }) => {
        const params = new URLSearchParams({
            q: brand,
            limit: '60',
            fields: '+attribute_values,+attribute_values.attribute_type',
    ...
    server.registerTool('list_articles', {
        ...
        inputSchema: {
            limit: zod_1.z.number().int().min(1).max(50).default(10).describe('Number of articles'),
        },
    }, async ({ limit = 10 }) => {
